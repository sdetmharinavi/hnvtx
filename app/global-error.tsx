...
export default function GlobalError({
  ...
  useEffect(() => {
    ...
    // If it's a chunk error, force a reload immediately to get new assets
    if (isChunkError) {
      ...
      <body className="bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 font-sans min-h-screen flex items-center justify-center">
        <div className="max-w-md w-full p-8 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-red-100 dark:border-red-900/30 text-center">
          ...
          <h2 className="text-2xl font-bold mb-2">
            {isChunkError ? 'Updating Application...' : 'Critical System Error'}
          </h2>
          
          <p className="text-gray-500 dark:text-gray-400 mb-6">
            {isChunkError 
              ? 'A new version of the app is available. Refreshing your session...' 
          ...
          <button
            onClick={handleReset}
            className={`inline-flex items-center justify-center gap-2 w-full px-4 py-3 text-white rounded-lg font-medium transition-colors shadow-lg ${
              isChunkError 
