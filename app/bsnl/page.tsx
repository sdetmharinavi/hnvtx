...
export default function ScalableFiberNetworkDashboard() {
  ...
  const {
    sortedData: sortedSystems,
    handleSort: handleSystemSort, // eslint-disable-line @typescript-eslint/no-unused-vars
    getSortDirection: getSystemSortDirection, // eslint-disable-line @typescript-eslint/no-unused-vars
  } = useSorting({ data: data.systems, defaultSortKey: "system_name" });
  const {
    sortedData: sortedCables,
    handleSort: handleCableSort, // eslint-disable-line @typescript-eslint/no-unused-vars
    getSortDirection: getCableSortDirection, // eslint-disable-line @typescript-eslint/no-unused-vars
  } = useSorting({ data: data.ofcCables, defaultSortKey: "route_name" });

  const [selectedSystem, setSelectedSystem] = useState<BsnlSystem | null>(null);
  const [selectedCable, setSelectedCable] = useState<BsnlCable | null>(null); // eslint-disable-line @typescript-eslint/no-unused-vars
  const [allocationData, setAllocationData] = useState<AllocationSaveData | null>(null); // eslint-disable-line @typescript-eslint/no-unused-vars
  ...
  const systemColumns: Column<BsnlSystem>[] = useMemo(
    () => [
      {
        key: "system_name",
        title: "System Name",
        ...
        sortable: true,
        render: (value: unknown, record: BsnlSystem) => (
          <div>
            <div className='font-medium text-gray-900 dark:text-white'>{record.system_name}</div>
            <div className='text-sm text-gray-500 dark:text-gray-400'>
              {record.system_type_name}
            </div>
          </div>
        ),
      },
      ...
      { key: "node_name", title: "Node", dataIndex: "node_name", sortable: true },
      {
        ...
        sortable: true,
        render: (value: unknown, record: BsnlSystem) => (
          <code className='text-xs'>{record.ip_address as string}</code>
        ),
      },
      {
  ...
  const cableColumns: Column<BsnlCable>[] = useMemo(
    () => [
      {
        key: "route_name",
        title: "Route Name",
        ...
        sortable: true,
        render: (value: unknown, record: BsnlCable) => (
          <div>
            <div className='font-medium text-gray-900 dark:text-white'>{record.route_name}</div>
            <div className='text-sm text-gray-500 dark:text-gray-400'>{record.asset_no}</div>
          </div>
        ),
      },
      {
      ...
      },
      {
        ...
        sortable: true,
        render: (value: unknown, record: BsnlCable) =>
          `${record.capacity}F / ${record.current_rkm?.toFixed(1)}km`,
      },
      {
        key: "endpoints",
        title: "Endpoints",
        ...
        render: (value: unknown, record: BsnlCable) => (
          <div className='text-sm'>
            {record.sn_name} → {record.en_name}
          </div>
        ),
      },
      { key: "ofc_owner_name", title: "Owner", dataIndex: "ofc_owner_name", sortable: true },
    ],
    []
  );

  // THE FIX: Define actions for the DataTable instead of using onRowClick.
  const tableActions: TableAction<"v_systems_complete" | "v_ofc_cables_complete">[] = useMemo(
    () => [
      {
        key: "view-details",
        label: "View Details",
        icon: <Eye size={16} />,
        onClick: (record) => {
          if ("system_name" in record) {
            // Type guard to check if it's a BsnlSystem
