...
import { FiActivity, FiUpload, FiGitCommit, FiTrash2 } from 'react-icons/fi';
...
export default function GlobalOfcConnectionsPage() {
  ...
  // THE FIX: Set `showCableContext` to true to ensure `ofc_route_name` isn't hidden
  const columns = OfcDetailsTableColumns(fibers, true);
  ...
  const tableActions: TableAction<'v_ofc_connections_complete'>[] = useMemo(
    () => [
      ...
      //   onClick: (record) => window.open(`/dashboard/ofc/${record.ofc_id}`, '_blank'),
      ...
      return (
        <>
          ...
          <Button
            ...
            title='Trace Path'>
            <FiGitCommit className='w-4 h-4 mr-1' /> Trace
          </Button>
        </>
      );
    },
  ...
  const renderItem = useCallback(
    (record: V_ofc_connections_completeRowSchema) => {
      return <FiberConnectionCard fiber={record} actions={getCardActions(record)} />;
    },
    [getCardActions],
  );
  ...
  return (
    <DashboardPageLayout
      header={{
        ...
        description: 'View and search all physical fiber connections across all cables.',
        icon: <FiActivity />,
      ...
      crud={crud} // Pass crud to layout for auto-wiring
