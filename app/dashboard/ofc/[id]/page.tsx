...
import { PageSpinner, ConfirmModal, Button, ErrorDisplay } from '@/components/common/ui';
...
export default function OfcCableDetailsPage() {
  ...
  const handleVerifyAndCreateFibers = useCallback(() => {
    if (routeDetails?.route && utilization) {
      if (
        ...
        routeDetails.route.capacity == null ||
        routeDetails.route.capacity <= 0
      ) {
        toast.error('Cable data is missing required fields (id/sn/en/capacity).');
        return;
      }
      ...
      if (expectedCount && existingCount < expectedCount) {
        createConnectionsMutation.mutate({
          cable: {
            id: routeDetails.route.id,
            capacity: routeDetails.route.capacity,
            sn_id: routeDetails.route.sn_id,
            en_id: routeDetails.route.en_id,
          },
        });
      } else {
  ...
  // If route details failed, check why.
  if (isRouteDetailsError || !routeDetails?.route) {
    // Is it a 404 or a network error?
    const errorMsg = routeDetailsError?.message || 'Cable data could not be loaded.';

    // If it's explicitly "Route not found" or "404", show the not found component
    ...
    // Otherwise show a generic error with retry
    return (
      <ErrorDisplay
        error={errorMsg}
