...
export default function EmployeesPage() {
  ...
  const filterConfigs = useMemo(
    () =>[
      { key: 'employee_designation_id', type: 'multi-select' as const, options: desOptions, isLoading: loadingDes },
      { key: 'maintenance_terminal_id', type: 'multi-select' as const, options: areaOptions, isLoading: loadingAreas },
    ],
  ...
  const columns = useMemo(() => getEmployeeTableColumns().map(c => ({...c, editable: false})),[]);
  ...
  const headerStats = useMemo<StatProps[]>(() => {
    ...
    return[
      { value: totalCount, label: 'Total Employees', color: 'default', onClick: () => filters.setFilters((prev) => { const next = { ...prev }; delete next.status; return next; }), isActive: !currentStatus },
      { value: activeCount, label: 'Working', color: 'success', onClick: () => filters.setFilters((prev) => ({ ...prev, status: 'true' })), isActive: currentStatus === 'true' },
      { value: inactiveCount, label: 'Retired / Inactive', color: 'danger', onClick: () => filters.setFilters((prev) => ({ ...prev, status: 'false' })), isActive: currentStatus === 'false' },
    ];
  },[totalCount, activeCount, inactiveCount, filters.filters.status, filters.setFilters]);

  const getAvatarColor = (name: string) => {
    const colors =['from-blue-500 to-blue-600', 'from-purple-500 to-purple-600', 'from-pink-500 to-pink-600', 'from-green-500 to-green-600', 'from-orange-500 to-orange-600', 'from-cyan-500 to-cyan-600'];
    const index = name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    return colors[index % colors.length];
  };

  const renderItem = useCallback((emp: V_employeesRowSchema) => (
      <GenericEntityCard
        key={emp.id}
        entity={emp}
        title={emp.employee_name || 'Unknown Employee'}
        subtitle={emp.employee_designation_name || 'No Designation'}
        initials={(emp.employee_name || '?').split(' ').map((n) => n[0]).slice(0, 2).join('').toUpperCase()}
        avatarColor={getAvatarColor(emp.employee_name || '')}
        status={emp.status}
        showStatusLabel={false}
        subBadge={emp.employee_pers_no ? (<span className='inline-flex items-center text-xs bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2.5 py-1 rounded-full font-medium'>ID: {emp.employee_pers_no}</span>) : null}
        dataItems={[
  ...
  const renderGrid = useCallback(() => (
      <DataGrid data={employees} renderItem={renderItem} isLoading={isLoading} isEmpty={employees.length === 0 && !isLoading} pagination={{ current: pagination.currentPage, pageSize: pagination.pageLimit, total: totalCount, onChange: (page, pageSize) => { pagination.setCurrentPage(page); pagination.setPageLimit(pageSize); } }} />
  ...
  return (
    <DashboardPageLayout
      header={{ title: 'Employee Directory', description: 'View team members and contact details.', icon: <FiUsers />, actions: headerActions, stats: headerStats, isLoading: isInitialLoad, isFetching: isFetching }}
      ...
      tableProps={{
        ...
        selectable: false,
        pagination: { current: pagination.currentPage, pageSize: pagination.pageLimit, total: totalCount, showSizeChanger: true, onChange: (page, pageSize) => { pagination.setCurrentPage(page); pagination.setPageLimit(pageSize); } },
      }}
