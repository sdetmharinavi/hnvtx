...
import { Copy, Database as DatabaseIcon, Server, ExternalLink, Activity, Tag, Hash, Info } from 'lucide-react';
...
export default function ServicesPage() {
  ...
  const enhancedHeaderActions: ActionButton[] = [...headerActions, {
  ...
  const renderGrid = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {data.map((service) => {
          const isDup = duplicateSet.has(`${service.name?.trim().toLowerCase()}|${service.link_type_name?.trim().toLowerCase()}`);
          ...
          dataItems.push({ 
            ...
            value: (
                <div className="flex items-center gap-1 text-xs">
                    ...
                    <TruncateTooltip text={service.end_node_name || '?'} className="font-medium max-w-[80px]" />
                </div>
            )
          });
                        ...
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-2 rounded-lg border border-blue-100 dark:border-blue-800">
                             ...
                             <div className="flex flex-wrap gap-1">
                                ...
                                {allocatedSystems.length > 2 && (
                                    <span className="text-[10px] text-blue-500">+{allocatedSystems.length - 2}</span>
                                )}
                             </div>
                        </div>
                    )}
