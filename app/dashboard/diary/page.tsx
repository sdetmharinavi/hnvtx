...
export default function DiaryPage() {
  ...
  const {
    ...
    dataQueryHook: () => ({ data: [], totalCount: 0, activeCount: 0, inactiveCount: 0, isLoading: false, error: null, refetch: () => {} }),
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-0.5">
                      {notesForSelectedDay.length === 0 
                        ? 'No entries yet' 
                        : `${notesForSelectedDay.length} ${notesForSelectedDay.length === 1 ? 'entry' : 'entries'}`
                      }
                    </p>
                  </div>
                </div>
