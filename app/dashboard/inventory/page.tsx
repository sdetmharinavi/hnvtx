...
import { useCrudManager, UseCrudManagerReturn } from '@/hooks/useCrudManager';
import { useInventoryData, InventoryDataReturn } from '@/hooks/data/useInventoryData';
import { useIssueInventoryItem, useRestockInventoryItem } from '@/hooks/inventory-actions';
import { useInventoryExcelUpload } from '@/hooks/database/excel-queries/useInventoryExcelUpload';
import { useLookupTypeOptions, useActiveNodeOptions } from '@/hooks/data/useDropdownOptions';
import { V_inventory_itemsRowSchema, Inventory_itemsInsertSchema } from '@/schemas/zod-schemas';
import { formatCurrency } from '@/utils/formatters';
...
const RestockItemModal = dynamic(
  () => import('@/components/inventory/RestockItemModal').then((mod) => mod.RestockItemModal),
  { loading: () => <PageSpinner text='Loading Restock Form...' /> },
);
...
export default function InventoryPage() {
  ...
  const [itemToRestock, setItemToRestock] = useState<V_inventory_itemsRowSchema | null>(null);
  ...
  const { mutate: issueItem, isPending: isIssuing } = useIssueInventoryItem();
  const { mutate: restockItem, isPending: isRestocking } = useRestockInventoryItem();
  ...
  const handleOpenRestockModal = (record: V_inventory_itemsRowSchema) => {
    setItemToRestock(record);
  ...
  const tableActions = useMemo(() => {
    ...
    if (canEdit) {
      ...
      standardActions.unshift({
        ...
        onClick: (r) => handleOpenRestockModal(r),
        variant: 'secondary',
      });
    }
    standardActions.unshift({
              ...
              {canEdit && (
                <>
                  <Button
                    ...
                    title='Add Stock'
                    className='bg-green-600 hover:bg-green-700 focus:ring-green-500 text-white border-green-700'
                  >
                  ...
                  <Button
                    ...
                    disabled={quantity <= 0}
                    title='Issue Stock'
                    className='bg-orange-600 hover:bg-orange-700 focus:ring-orange-500 text-white border-orange-700'
                  >
                    <FiMinusCircle className='w-4 h-4' />
                  </Button>
                </>
