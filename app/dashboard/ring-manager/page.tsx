...
const mapNodeData = (node: any): RingMapNode | null => {
...
const RingAssociatedSystemsView = ({ ringId }: { ringId: string }) => {
  ...
  if (isLoading) return <div className='py-4 text-center text-sm text-gray-500'>Loading systems...</div>;
  if (systems.length === 0) return <div className='text-sm text-gray-500 italic py-2'>No systems associated with this ring yet.</div>;
  ...
  return (
    <div className='space-y-2 max-h-96 overflow-y-auto pr-1 custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-2'>
      {systems.map((system) => {
        ...
        return (
          <div key={system.id} className='flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700/50 rounded-md border border-gray-200 dark:border-gray-600'>
            <div>
              <div className='flex items-center gap-2'>
                <span className='font-medium text-sm text-gray-900 dark:text-gray-100'>{system.system_name}</span>
                {system.ip_address && <span className='font-mono text-xs text-gray-500 dark:text-gray-400'>/ {system.ip_address}</span>}
              </div>
              <div className='flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 mt-1.5'>
                <span className='font-mono bg-gray-200 dark:bg-gray-600 px-1.5 py-0.5 rounded text-[10px] font-bold'>#{system.order_in_ring ?? '?'}</span>
                {system.is_hub ? (
                  <span className='text-blue-700 dark:text-blue-300 font-semibold flex items-center gap-1 bg-blue-100 dark:bg-blue-900/40 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wide'><FiArrowRightCircle className='w-3 h-3' /> Hub</span>
                ) : (
                  <span className='text-purple-700 dark:text-purple-300 font-medium flex items-center gap-1 bg-purple-100 dark:bg-purple-900/40 px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wide'><FiGitMerge className='w-3 h-3' /> Spur {parentName && <span className='text-gray-500 dark:text-gray-400 ml-1 lowercase tracking-normal'>via {parentName}</span>}</span>
                )}
              </div>
            </div>
          </div>
...
export default function RingManagerPage() {
  ...
  const dynamicStats = useMemo<DynamicStats>(() => {
    return (manager as any).stats || { total: 0, totalNodes: 0, spec: { issued: 0, pending: 0 }, ofc: { ready: 0, partial: 0, pending: 0 }, bts: { onAir: 0, pending: 0, nodesOnAir: 0, configuredCount: 0 } };
  }, [manager]);
  ...
  const ringTypesData = useMemo(() => (ringTypesRaw || []) as Lookup_typesRowSchema[],[ringTypesRaw]);
  const maintenanceAreasData = useMemo(() => (maintenanceAreasRaw ||[]) as Maintenance_areasRowSchema[], [maintenanceAreasRaw]);

  const handleViewDetails = useCallback((record: V_ringsRowSchema) => {
    ...
    return[
      {
        ...
        onClick: () => filters.setFilters((prev) => { const next = { ...prev }; delete next.ofc_status; delete next.spec_status; delete next.bts_status; return next; }),
  ...
  }, [dynamicStats, filters.filters, filters.setFilters]);
  ...
  const dynamicFilterConfig: EntityConfig<RingEntity> = useMemo(() => ({
    ...
    filterOptions:[
      ...
      {
        key: 'ofc_status', label: 'OFC Status', type: 'select' as const,
        options:[{ value: 'Ready', label: 'Ready' }, { value: 'Pending', label: 'Pending' }, { value: 'Partial Ready', label: 'Partial Ready' }],
      },
      {
        key: 'bts_status', label: 'Working Status', type: 'select' as const,
        options:[{ value: 'On-Air', label: 'On-Air' }, { value: 'Pending', label: 'Pending' }, { value: 'Configured', label: 'Configured' }],
      },
    ].map((opt) => {
      if (opt.key === 'ring_type_id') return { ...opt, options: ringTypesData.map(t => ({ value: t.id!, label: t.name! })) };
      if (opt.key === 'maintenance_terminal_id') return { ...opt, options: maintenanceAreasData.map(m => ({ value: m.id!, label: m.name! })) };
      return opt;
    }),
  ...
  const uiFilters = useMemo<Record<string, string>>(() => {
    ...
    Object.keys(src).forEach((k) => {
      ...
      out[k] = typeof v === 'object' && 'value' in v ? String((v as any).value) : String(v);
    });
    return out;
  },[filters.filters]);

  // --- MAP RENDER LOGIC FOR SELECTED RING ---
  // The map logic fetches sub-data independently.
  ...
  const { data: rawNodes } = useLocalFirstQuery<'v_ring_nodes'>({
    ...
    onlineQueryFn: async () => {
      ...
      const { data } = await supabase.rpc('get_paged_data', { p_view_name: 'v_ring_nodes', p_limit: 1000, p_offset: 0, p_filters: rpcFilters, p_order_by: 'order_in_ring' });
      return (data as any)?.data ||[];
    },
    localQueryFn: () => selectedRingId ? localDb.v_ring_nodes.where('ring_id').equals(selectedRingId).toArray() : Promise.resolve([]),
    dexieTable: localDb.v_ring_nodes,
    enabled: !!selectedRingId,
  });

  const mappedNodes = useMemo(() => (rawNodes || []).map(mapNodeData).filter(Boolean) as RingMapNode[],[rawNodes]);
  const ringNodeIds = useMemo(() => (rawNodes ||[]).map(n => n.node_id).filter(Boolean) as string[], [rawNodes]);
  ...
  const { data: ringCables } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase.rpc('get_paged_data', { p_view_name: 'v_ofc_cables_complete', p_limit: 1000, p_offset: 0, p_filters: { or: `sn_id IN (${idList}) OR en_id IN (${idList})` }});
      const allCables = (data?.data ||[]);
      const ringNodeSet = new Set(ringNodeIds);
      return allCables.filter((c: any) => ringNodeSet.has(c.sn_id) && ringNodeSet.has(c.en_id));
    },
  ...
  const { data: pathConfigs } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('logical_paths').select('id, name, start_node_id, end_node_id, source_system_id, destination_system_id').eq('ring_id', selectedRingId!);
      return data ||[];
    },
    enabled: !!selectedRingId,
  });
  ...
  const { potentialSegments, spurConnections } = useMemo(() => {
    ...
    const hubs = mappedNodes.filter(n => n.is_hub).sort((a, b) => (a.order_in_ring || 0) - (b.order_in_ring || 0));
  ...
  const { allSolidLines, configMap } = useMemo(() => {
    ...
    const activeTopology = potentialSegments.filter(([s, e]) => !disabledKeys.has(`${s.id}-${e.id}`) && !disabledKeys.has(`${e.id}-${s.id}`));
    ...
    if (pathConfigs && pathConfigs.length > 0) {
      pathConfigs.forEach((pc: any) => {
        ...
        if (start && end) {
          ...
          map[sortedKey].push({ connectionId: pc.id, cableName: pc.name, color: COLOR_PALETTE[lines.length % COLOR_PALETTE.length] });
        }
      });
    }
    ...
    activeTopology.forEach(([start, end]) => {
      ...
      if (!map[sortedKey] || map[sortedKey].length === 0) {
        ...
        if (ringCables) {
          const matching = ringCables.find(c => (c.sn_id === start.node_id && c.en_id === end.node_id) || (c.sn_id === end.node_id && c.en_id === start.node_id));
          if (matching) cableName = matching.route_name || 'Link';
        }
        if (!map[sortedKey]) map[sortedKey] = [];
      ...
      {selectedRingId && mappedNodes.length > 0 && (
         <div className="flex justify-end mt-4 mb-2">
            <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                <button onClick={() => setViewMode('map')} className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'map' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}><FiMap className="inline mr-1"/> Map</button>
                <button onClick={() => setViewMode('schematic')} className={`px-3 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'schematic' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-600 hover:text-gray-900'}`}><FiGrid className="inline mr-1"/> Schematic</button>
            </div>
         </div>
      )}
      ...
      {selectedRingId && mappedNodes.length > 0 && (
          <div className="fixed inset-4 md:inset-10 lg:inset-20 z-50 bg-white dark:bg-gray-900 rounded-xl shadow-2xl overflow-hidden flex flex-col border border-gray-200 dark:border-gray-700">
             <div className="flex justify-between items-center p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                <h2 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2"><GiLinkedRings /> {ringDetails?.name || 'Ring Map'}</h2>
                <Button variant="outline" onClick={() => viewModal.close()}>Close Viewer</Button>
             </div>
             <div className="flex-1 relative">
                {viewMode === 'schematic' ? (
                  <MeshDiagram nodes={mappedNodes} connections={allSolidLines} segmentConfigs={configMap} />
                ) : (
                  <ClientRingMap nodes={mappedNodes.filter(n => n.lat != null)} solidLines={allSolidLines} dashedLines={spurConnections} segmentConfigs={configMap} showControls />
                )}
             </div>
          </div>
      )}
