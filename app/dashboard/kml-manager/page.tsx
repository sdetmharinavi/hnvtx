...
import { FiMap, FiFileText, FiDownload, FiRefreshCw } from 'react-icons/fi';
...
export default function KmlManagerPage() {
  const { kmlFiles, isLoading, refetch } = useKmlManager();
  ...
  return (
    <div className="p-4 md:p-6 h-[calc(100vh-64px)] flex flex-col space-y-4">
      <PageHeader
        title="KML Viewer"
        description="Visualize Google Earth KML network routes."
        icon={<FiMap />}
        actions={[
          {
            label: 'Refresh',
      ...
      <div className="flex flex-col lg:flex-row gap-4 flex-1 min-h-0">
        ...
        <div className="w-full lg:w-1/3 xl:w-1/4 bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm flex flex-col overflow-hidden">
          <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/30 flex justify-between items-center">
            <h3 className="font-semibold text-gray-700 dark:text-gray-200">Available Files</h3>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
            {isLoading ? (
              ...
              <div className="text-center py-12 px-4">
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm">No KML files available.</p>
              </div>
            ) : (
              kmlFiles.map((file) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 rounded-lg border cursor-pointer transition-all group relative ${isSelected ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 shadow-sm' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-gray-600'}`}
                  >
                    <div className="flex items-start gap-3">
                      <div className={`p-2 rounded-md ${isSelected ? 'bg-blue-200 dark:bg-blue-800 text-blue-700 dark:text-blue-200' : 'bg-gray-100 dark:bg-gray-700 text-gray-500'}`}>
                        <FiFileText size={18} />
                      </div>
                    ...
                    <div className={`absolute right-2 top-2 flex gap-1 bg-white/90 dark:bg-gray-800/90 p-1 rounded-md shadow-sm backdrop-blur-sm border border-gray-100 dark:border-gray-700 transition-opacity duration-200 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                      <button onClick={(e) => { e.stopPropagation(); handleDownload(file.downloadUrl, fileName); }} className="p-1.5 text-gray-500 hover:text-blue-600 rounded transition-colors" title="Download">
                        <FiDownload size={14} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border dark:border-gray-700 shadow-sm p-1 relative flex flex-col min-h-[400px]">
          {!selectedKml ? (
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 bg-gray-50 dark:bg-gray-900/50 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700 m-4">
            ...
            <>
              <div className="absolute top-4 left-14 z-400 bg-white/90 dark:bg-gray-900/90 backdrop-blur px-3 py-1.5 rounded-md shadow-md border text-sm font-medium">
