...
export default function ExpensesPage() {
  ...
  const { mutate: exportAdvances, isPending: isExportingAdvances } = useTableExcelDownload(supabase, 'v_advances_complete');
  const { mutate: exportExpenses, isPending: isExportingExpenses } = useTableExcelDownload(supabase, 'v_expenses_complete');
  ...
  const advanceStats: StatProps[] = useMemo(() => {
    ...
    return[
      { value: advanceCrud.totalCount, label: 'All Advances', onClick: () => advanceCrud.filters.setFilters((prev) => { const next = { ...prev }; delete next.status; return next; }), isActive: !currentStatus },
      { value: activeCount, label: 'Active', color: 'success', onClick: () => advanceCrud.filters.setFilters((prev) => ({ ...prev, status: 'active' })), isActive: currentStatus === 'active' },
    ];
  ...
  const handleExportAdvances = useCallback(() => {
    ...
    exportAdvances({ fileName: `Advances_${new Date().toISOString().split('T')[0]}.xlsx`, sheetName: 'Advances', filters: advanceCrud.filters.filters, columns, wrapText: true, autoFitColumns: true });
  ...
  const handleExportExpenses = useCallback(() => {
    ...
    exportExpenses({ fileName: `Expenses_${new Date().toISOString().split('T')[0]}.xlsx`, sheetName: 'Expenses', filters: expenseCrud.filters.filters, columns, wrapText: true, autoFitColumns: true });
  }, [exportExpenses, expenseCrud.filters.filters]);

  const advanceColumns: Column<Row<'v_advances_complete'>>[] = useMemo(() =>[
    { key: 'req_no', title: 'Request No', dataIndex: 'req_no', sortable: true, width: 140, render: (v) => <span className='font-mono font-medium text-sm'>{v as string}</span> },
    { key: 'employee_name', title: 'Employee', dataIndex: 'employee_name', sortable: true, width: 180 },
    { key: 'advance_date', title: 'Date Issued', dataIndex: 'advance_date', sortable: true, width: 120, render: (v) => <span className='text-sm'>{formatDate(v as string, { format: 'dd-mm-yyyy' })}</span> },
    { key: 'total_amount', title: 'Amount', dataIndex: 'total_amount', sortable: true, width: 120, render: (v) => <span className='font-bold text-sm'>{formatCurrency(Number(v))}</span> },
    { key: 'spent_amount', title: 'Spent', dataIndex: 'spent_amount', width: 120, render: (v) => <span className='text-gray-600 dark:text-gray-400 text-sm'>{formatCurrency(Number(v))}</span> },
  ...
  const renderAdvanceItem = useCallback((item: V_advances_completeRowSchema) => {
    ...
    return (
      <GenericEntityCard
        ...
        dataItems={[
          { label: 'Starting Date', value: item.advance_date ? formatDate(item.advance_date) : 'N/A', icon: FiList },
          { label: 'Amount', value: formatCurrency(total), icon: FiPieChart },
          { label: 'Spent', value: formatCurrency(spent), icon: FiPieChart },
        ]}
        customFooter={
          <div className='w-full space-y-1'>
            <div className='flex justify-between text-xs'><span>Utilization</span><span>{percentage.value}%</span></div>
            <ProgressBar value={spent} max={total || 100} size='sm' variant={percentage.displayValue > 90 ? 'danger' : percentage.displayValue > 70 ? 'warning' : 'success'} />
  ...
  const expenseColumns: Column<Row<'v_expenses_complete'>>[] = useMemo(() =>[
    { key: 'expense_date', title: 'Date', dataIndex: 'expense_date', render: (v) => <span className='text-sm'>{formatDate(v as string)}</span>, sortable: true, width: 120 },
    { key: 'used_by', title: 'Used By', dataIndex: 'used_by', width: 150, render: (v) => <span className='text-sm'>{v as string}</span> },
    { key: 'category', title: 'Category', dataIndex: 'category', sortable: true, width: 120 },
    { key: 'amount', title: 'Amount', dataIndex: 'amount', render: (v) => <span className='text-sm font-semibold'>{formatCurrency(Number(v))}</span>, width: 120 },
    { key: 'description', title: 'Description', dataIndex: 'description', width: 300, render: (v) => <TruncateTooltip text={v as string} /> },
  ],[]);

  const activeAdvanceId = expenseCrud.filters.filters.advance_id ? String(expenseCrud.filters.filters.advance_id) : null;
      ...
      {activeTab === 'advances' ? (
        <DashboardPageLayout<'v_advances_complete'>
          header={{
            ...
            stats: advanceStats,
            actions:[
              { label: 'Refresh', onClick: handleRefresh, variant: 'outline', leftIcon: <FiRefreshCw className={isSyncing ? 'animate-spin' : ''} />, disabled: isLoading || isSyncing, hideTextOnMobile: true },
          ...
          tableProps={{
            tableName: 'v_advances_complete', data: advanceCrud.data, columns: advanceColumns, loading: advanceCrud.isLoading, searchable: false, filterable: false,
            actions: createStandardActions({ onView: (record) => { if (record.id) { expenseCrud.filters.setFilters({ advance_id: record.id }); setActiveTab('expenses'); } } }),
          }}
        />
      ) : (
            ...
            activeAdvanceId ? (
              <div className='bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3 mb-4 flex items-center justify-between'>
                <div className='flex items-center gap-2 text-blue-800 dark:text-blue-200'>
                  <FiFilter /><span className='text-sm font-medium'>Viewing expenses for Advance: <strong>{activeAdvanceDetails?.req_no || activeAdvanceId}</strong></span>
                </div>
                <Button size='sm' variant='ghost' onClick={() => expenseCrud.filters.setFilters((prev) => { const next = { ...prev }; delete next.advance_id; return next; })}><FiX className='mr-1' /> Clear Filter</Button>
              </div>
            ) : null
          }
          renderTable={() => (
            <DataTable
              data={expenseCrud.data} columns={expenseColumns} tableName='v_expenses_complete' loading={expenseCrud.isLoading} searchable={false} filterable={false}
              pagination={{ current: expenseCrud.pagination.currentPage, pageSize: expenseCrud.pagination.pageLimit, total: expenseCrud.totalCount, onChange: (p, s) => { expenseCrud.pagination.setCurrentPage(p); expenseCrud.pagination.setPageLimit(s); } }}
