...
export default function EFilesPage() {
  ...
  // THE FIX: Provide explicit pagination state to handle the 2000 row payload gracefully
  ...
  // THE FIX: Slice the final array based on pagination state so the UI doesn't render thousands of DOM nodes
  const paginatedFiles = useMemo(() => {
    const start = (currentPage - 1) * pageLimit;
    return filteredFiles.slice(start, start + pageLimit);
  }, [filteredFiles, currentPage, pageLimit]);
        ...
        <GenericEntityCard
          ...
          dataItems={[
            { icon: User, label: 'Started By', value: file.initiator_name },
            {
              ...
              value: file.current_holder_name,
            },
            {
              icon: Clock,
              label: 'Last Action',
  ...
  const renderGrid = useCallback(
    () => (
      <DataGrid
        // THE FIX: Use paginated data instead of full set
        data={paginatedFiles}
        renderItem={renderItem}
        isLoading={isLoading}
        isEmpty={paginatedFiles.length === 0 && !isLoading}
        // THE FIX: Add explicit pagination controls for the Grid view
        pagination={{
          current: currentPage,
          pageSize: pageLimit,
    ...
    [paginatedFiles, renderItem, isLoading, currentPage, pageLimit, filteredFiles.length],
  );
  ...
  return (
    <DashboardPageLayout
      ...
      tableProps={{
        ...
        searchable: false,
        // THE FIX: Plumb the proper pagination controls
        pagination: {
          current: currentPage,
          pageSize: pageLimit,
          total: filteredFiles.length,
