...
export default function OnboardingFormEnhanced() {
  ...
  const { profile, isLoading: isProfileLoading, error: profileError, refetch } = useUser();
  ...
  const { mutate: updateProfile, isPending: isUpdatePending } = useTableUpdate(
    ...
    'user_profiles',
    {
      onSuccess: (data) => {
        ...
        // THE FIX: Redirect to dashboard after successful update
  ...
  const onSubmit = (data: User_profilesUpdateSchema) => {
    ...
    for (const key in dirtyFields) {
      if (Object.prototype.hasOwnProperty.call(dirtyFields, key)) {
        ...
        (updates as Record<string, any>)[typedKey] =
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='space-y-6 w-full max-w-6xl mx-auto'>
