...
export function AreaFormModal({
  ...
  const { form, isEditMode } = useFormModal<
    Maintenance_areasInsertSchema,
    MaintenanceAreaWithRelations
  >({
    isOpen,
    schema: maintenance_areasInsertSchema,
    record: area,
    defaultValues: {
      name: '',
      code: '',
    ...
    mapRecord: (record) => ({
      name: record.name,
      code: record.code,
      area_type_id: record.area_type_id,
  ...
  // Reset manual code edit state when modal opens
  useEffect(() => {
    if (isOpen) {
      setIsCodeManuallyEdited(!!area);
    }
  }, [isOpen, area]);
