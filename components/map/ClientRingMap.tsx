...
// --- HELPER: Create HTML String for DivIcon (Lightweight replacement for renderToStaticMarkup) ---
  ...
  if (ports.length > 0) {
    ...
    const portItems = visiblePorts
      .map((p) => {
        ...
        return `<div class="px-1 font-bold py-px text-[14px] font-mono rounded border shadow-sm flex items-center gap-1 backdrop-blur-xs whitespace-nowrap" style="background-color: ${p.color}; color: ${textColor}; border-color: rgba(255,255,255,0.3)"><span>${p.port}</span></div>`;
      })
      .join("");
    ...
    portsHtml = `<div class="mt-0.5 flex flex-row gap-px items-center justify-center">${portItems}${moreItem}</div>`;
  }

  return `
    <div class="flex flex-col items-center cursor-grab active:cursor-grabbing transform -translate-y-full -translate-x-1/2 mt-[-10px]">
      <div class="px-1.5 py-0.5 text-[14px] font-bold rounded border shadow-sm backdrop-blur-xs whitespace-nowrap z-10 ${bgClass} ${borderClass}">
        ${name} ${ip ? `- ${ip}` : ""}
      </div>
      ${portsHtml}
    </div>
  ...
  return (
    <div className={mapContainerClass}>
      ...
      <MapContainer
        ...
        {/* Node Rendering - Split into Marker and Label */}
        {displayNodes.map((node: DisplayNode<RingMapNode>, i) => {
          ...
          // Determine label position (User drag state -> Jittered Node position)
          const labelPos: [number, number] = labelPositions[node.id!] || [
            node.displayLat,
            node.displayLng,
          ];
          ...
          const labelIcon = L.divIcon({
            ...
            className: "bg-transparent border-none", // Override default Leaflet white box
            iconSize: [0, 0], // Let content dictate size
            iconAnchor: [0, 0], // Centering handled in HTML transform
          });
