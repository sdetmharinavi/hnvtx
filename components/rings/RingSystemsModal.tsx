...
const useRingSystemsData = (ring: V_ringsRowSchema | null) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Fetch all systems in maintenance area to filter for available
...
export function RingSystemsModal({ isOpen, onClose, ring }: RingSystemsModalProps) {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      // THE FIX: Removed localDb sync calls
      await queryClient.invalidateQueries({ queryKey: ['rings-manager-data'] });
      await queryClient.invalidateQueries({ queryKey: ['ring-systems-data'] });
