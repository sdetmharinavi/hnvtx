...
export const EFileCard: React.FC<EFileCardProps> = ({
  ...
  const getPriorityColor = (p: string | null) => {
    switch (p) {
      case 'immediate':
        return 'bg-red-100 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-300 dark:border-red-800 animate-pulse';
      case 'urgent':
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-start gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            {isClosed && (
              <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-gray-200 text-gray-600 dark:bg-gray-700 dark:text-gray-400">
                Closed
              </span>
            )}
          </div>
      ...
      <div className="p-4 flex-1 space-y-3">
        <div>
          <TruncateTooltip
            className="text-sm font-medium text-gray-800 dark:text-gray-200 line-clamp-2"
            text={file.subject || ''}
          />
        </div>
