...
type ConnectionOptionData = {
  ...
  connected_link_type_name: string | null; // Added
};
...
export const FiberAssignmentModal: React.FC<FiberAssignmentModalProps> = ({
  ...
  // 3. THE FIX: Replaced useOfflineQuery with standard useQuery
  const { data: connectionsData, isLoading: isLoadingConnections } = useQuery<ConnectionOptionData[]>({
    queryKey: ["active-system-connections-list"],
    queryFn: async () => {
    ...
    const mappedOptions = filtered
      ...
      .map((conn) => {
        ...
        const details = [
          ...
          conn.unique_id ? `UNIQUE ID: ${conn.unique_id}` : null,
        ]
          .filter(Boolean)
          .join(" | ");
