      ...
      <div className='px-3 sm:px-5 py-3 border-b border-gray-100 dark:border-gray-700/50 bg-linear-to-b from-gray-50/50 to-transparent dark:from-gray-900/20 pl-5'>
        <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-3'>
          <div className='flex items-center gap-2 sm:gap-3 flex-wrap'>
            {/* THE FIX: Added Parent Cable Link Badge */}
            {fiber.ofc_id && fiber.ofc_route_name && (
              <Link
                href={`/dashboard/ofc/${fiber.ofc_id}`}
                target='_blank'
                onClick={(e) => e.stopPropagation()}
                className='group/link flex items-center gap-1.5 text-xs font-bold text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded-md border border-blue-200 dark:border-blue-800/50 hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors'
                title='Open Parent Cable'
              >
                <Cable className='w-3.5 h-3.5 shrink-0' />
                <TruncateTooltip
                  text={fiber.ofc_route_name}
                  className='max-w-[120px] truncate group-hover/link:underline'
                />
                <ExternalLink className='w-3 h-3 opacity-50 group-hover/link:opacity-100 shrink-0' />
              </Link>
            )}
