...
import { FormInput, FormSearchableSelect, FormTextarea } from "@/components/common/ui/form/FormControls";
...
export function NodeFormModal({ isOpen, onClose, editingNode, onCreated, onUpdated }: NodeFormModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={""} size='full' visible= {false} className="h-screen w-screen transparent bg-gray-700 rounded-2xl">
      <FormCard title={isEdit ? "Edit Node" : "Add Node"} onSubmit={handleSubmit(onValidSubmit)} onCancel={handleClose} standalone>
        {/* Name */}
        <FormInput name='name' label='Node Name' register={register} error={errors.name} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* IP Address */}
        <FormInput name='ip_address' label='IP Address' register={register} error={errors.ip_address} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />
        ...
        {/* Site */}
        <FormInput name='site_id' label='Site' register={register} error={errors.site_id} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* Coordinates */}
        <FormInput name='latitude' label='Latitude' register={register} error={errors.latitude} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        <FormInput name='longitude' label='Longitude' register={register} error={errors.longitude} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* Order in Ring */}
        <FormInput name='order_in_ring' label='Order in Ring' register={register} error={errors.order_in_ring} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* VLAN */}
        <FormInput name='vlan' label='VLAN' register={register} error={errors.vlan} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* Builtup */}
        <FormTextarea name='builtup' label='Builtup Area' control={control} error={errors.builtup} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* Ports */}
        <FormInput name='east_port' label='East Port' register={register} error={errors.east_port} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        <FormInput name='west_port' label='West Port' register={register} error={errors.west_port} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* Remark */}
        <FormTextarea name='remark' label='Remark' control={control} error={errors.remark} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />

        {/* Status */}
        <FormInput name='status' label='Status' register={register} error={errors.status} disabled={submitting} className='dark:bg-gray-900 dark:text-gray-100' />
      </FormCard>
    </Modal>
  );
}
