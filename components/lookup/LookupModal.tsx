...
export function LookupModal({ isOpen, onClose, onLookupCreated, onLookupUpdated, editingLookup, category, categories }: LookupModalProps) {
  // Database hooks
  ...
  const submitButtonText = isEditMode ? (isSubmitting ? "Updating..." : "Update Lookup Type") : isSubmitting ? "Creating..." : "Create Lookup Type";
      ...
      <form onSubmit={handleSubmit(onValidSubmit)} className='space-y-4'>
        <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
          ...
          <div className='md:col-span-2'>
            ...
            {isEditMode || category ? (
              ...
              <div className='space-y-1'>
                <Input type='text' {...register("category")} disabled={true} className='bg-gray-50 dark:bg-gray-700 text-gray-500 dark:text-gray-400' />
                {errors.category && <p className='text-xs text-red-500 dark:text-red-400 mt-1'>{errors.category.message}</p>}
                <p className='text-xs text-gray-500 dark:text-gray-400'>{isEditMode ? "Category cannot be changed when editing" : "Category is set from parent"}</p>
              </div>
            ) : (
          ...
          <div className='md:col-span-2'>
            ...
            <Input
              ...
              disabled={isSubmitting}
              className={`${errors.name ? "border-red-300 dark:border-red-600" : ""} dark:bg-gray-800 dark:border-gray-600 dark:text-white dark:placeholder-gray-400`}
            />
          ...
          <div>
            ...
            <Input
              ...
              disabled={isSubmitting}
              className={`${errors.code ? "border-red-300 dark:border-red-600" : ""} dark:bg-gray-800 dark:border-gray-600 dark:text-white dark:placeholder-gray-400`}
            />
          ...
          {/* Sort Order Field */}
          <div>
            <label className='mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300'>Sort Order</label>
            <Input
              type='number'
              ...
              disabled={isSubmitting}
              min='0'
              className={`${errors.sort_order ? "border-red-300 dark:border-red-600" : ""} dark:bg-gray-800 dark:border-gray-600 dark:text-white`}
            />
          ...
          <div className='md:col-span-2 space-y-3'>
            <div className='flex items-center'>
              ...
              />
              <label htmlFor='is_system_default' className='ml-2 text-sm text-gray-700 dark:text-gray-300'>
                System Default
                {isEditMode && editingLookup?.is_system_default && <span className='ml-1 text-xs text-gray-500 dark:text-gray-400'>(cannot be changed)</span>}
              </label>
            </div>
        ...
        {isEditMode ? (
          <div className='rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-3'>
            <h4 className='mb-1 text-sm font-medium text-amber-900 dark:text-amber-200'>Edit Mode Notes:</h4>
          ...
          <div className='rounded-md border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 p-3'>
            <h4 className='mb-1 text-sm font-medium text-blue-900 dark:text-blue-200'>{category ? "Adding to Lookup" : "Creating New Lookup Type"}</h4>
            <ul className='space-y-1 text-xs text-blue-800 dark:text-blue-200/80'>
              ...
              {!category && <li>• Select an existing category or create a new one</li>}
            </ul>
          </div>
        )}
        ...
        <div className='flex justify-end gap-2 pt-4'>
          <Button type='button' variant='outline' onClick={onClose} disabled={isSubmitting} className='dark:border-gray-600 dark:hover:bg-gray-700'>
            Cancel
          </Button>
