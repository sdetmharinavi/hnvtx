...
import {
  useJcSplicingDetails,
  useManageSplice,
  useAutoSplice,
  useSyncPathUpdates,
} from '@/hooks/database/route-manager-hooks';
...
const useNormalizedSplicingDetails = (
  junctionClosureId: string | null
): {
...
export const FiberSpliceManager: React.FC<FiberSpliceManagerProps> = ({
  junctionClosureId,
  canEdit,
}) => {
  const {
    normalizedData: spliceDetails,
    isLoading,
    isError,
    error,
  } = useNormalizedSplicingDetails(junctionClosureId);
  ...
  const [selectedFiber, setSelectedFiber] = useState<{ segmentId: string; fiberNo: number } | null>(
    null
  );
  ...
  useEffect(() => {
    if (pendingSpliceAction?.type === 'auto' && pendingSpliceAction.autoData && spliceDetails) {
      ...
      if (segment1 && segment2) {
        const availableFibers1 = segment1.fibers
          .filter((f) => f.status === 'available')
          .map((f) => f.fiber_no);
        const availableFibers2 = segment2.fibers
          .filter((f) => f.status === 'available')
          .map((f) => f.fiber_no);
  ...
  const handleFiberClick = (segmentId: string, fiberNo: number, status: FiberStatus) => {
    ...
    if (
      selectedFiber &&
      selectedFiber.segmentId === segmentId &&
      selectedFiber.fiberNo === fiberNo
    ) {
  ...
  const confirmSplice = async () => {
    ...
    if (pendingSpliceAction.type === 'manual' && pendingSpliceAction.manualData) {
      ...
      const { incomingSegmentId, incomingFiberNo, outgoingSegmentId, outgoingFiberNo } =
        pendingSpliceAction.manualData;
  ...
  if (!spliceDetails?.junction_closure) {
    return (
      <div className="p-4 text-gray-500">Select a Junction Closure to manage its splices.</div>
    );
  }
  ...
  const renderFiber = (fiber: FiberAtSegment, segmentId: string) => {
    const isSelected =
      selectedFiber?.segmentId === segmentId && selectedFiber.fiberNo === fiber.fiber_no;
    const isTargetable =
      canEdit &&
      Boolean(selectedFiber) &&
      selectedFiber?.segmentId !== segmentId &&
    ...
    return (
      <div
        ...
        onClick={() =>
          isTargetable
            ? handleTargetFiberClick(segmentId, fiber.fiber_no)
            : handleFiberClick(segmentId, fiber.fiber_no, fiber.status)
        }
        className={`flex items-center justify-between p-2 rounded-md transition-all duration-200 ${
          isSelected
            ? 'ring-2 ring-yellow-500 bg-yellow-100 dark:bg-yellow-900/40'
            : isTargetable
            ? 'cursor-pointer hover:bg-green-200 dark:hover:bg-green-800/50'
            : fiber.status === 'used_as_outgoing'
        ...
        <div className="flex items-center gap-2">
          ...
          {canEdit && (
            <Button
              ...
              leftIcon={
                syncPathUpdatesMutation.isPending ? (
      ...
      {selectedFiber && (
        <div className="p-3 mb-4 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700 rounded-lg text-center">
          <p className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
            Selected Fiber #{selectedFiber.fiberNo} from{' '}
            {segments_at_jc.find((s) => s.segment_id === selectedFiber.segmentId)?.segment_name}.
            Click an available fiber to create a splice.
          </p>
        </div>
      )}
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns }}>
        {segments_at_jc.map((segment, index) => (
          <div
            ...
            className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg border dark:border-gray-700"
          >
            ...
            {index < segments_at_jc.length - 1 && canEdit && (
              <Button
                size="xs"
                onClick={() =>
                  handleAutoSplice(segment.segment_id, segments_at_jc[index + 1].segment_id)
                }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <>
                <div className="mb-4">
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">
                    Splicing between: <strong>{pendingSpliceAction.autoData?.segment1Name}</strong>{' '}
                    ↔ <strong>{pendingSpliceAction.autoData?.segment2Name}</strong>
                  </p>
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    {autoSplicePairs.length} fiber pair{autoSplicePairs.length !== 1 ? 's' : ''}{' '}
                    will be spliced
                  </p>
                </div>
                ...
                <div className="mb-4 space-y-3">
                  <label className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    <input
                      ...
                      onChange={(e) =>
                        setAutoSpliceMode(e.target.value as 'uniform' | 'individual')
                      }
                      className="mt-1"
                    />
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-600 dark:text-gray-400">
                        Apply same loss to all splices
                      </div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-3 border rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-gray-600 dark:text-gray-400">
                        Set different loss for each splice
                      </div>
                    </div>
                  </label>
                </div>
                    ...
                    <div className="border dark:border-gray-700 rounded-lg overflow-hidden">
                      ...
                      <div className="max-h-64 overflow-y-auto">
                        {autoSplicePairs.map((pair, idx) => (
                          <div
                            key={idx}
                            className="px-3 py-2 grid grid-cols-3 gap-2 items-center border-t dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50"
                          >
            ...
            <div className="flex gap-3 justify-end">
              ...
              <Button
                onClick={confirmSplice}
                disabled={manageSpliceMutation.isPending || autoSpliceMutation.isPending}
              >
                {manageSpliceMutation.isPending || autoSpliceMutation.isPending
                  ? 'Creating...'
                  : pendingSpliceAction?.type === 'auto'
                  ? `Create ${autoSplicePairs.length} Splice${
                      autoSplicePairs.length !== 1 ? 's' : ''
                    }`
                  : 'Confirm Splice'}
              </Button>
            </div>
