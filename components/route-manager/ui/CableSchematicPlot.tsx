...
export const CableSchematicPlot: React.FC<CableSchematicPlotProps> = ({ routeDetails }) => {
  ...
  // THE FIX: Make the SVG width dynamic based on the number of points to prevent text collision
  const PADDING_X = 250;
  const BASE_WIDTH = 1200;
  // Calculate required width giving each node ~200px of breathing room
  const dynamicWidth = Math.max(BASE_WIDTH, points.length * 200 + PADDING_X * 2);
  const SVG_WIDTH = dynamicWidth;
  ...
  return (
    <div className='w-full overflow-x-auto bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 custom-scrollbar'>
      <div className='flex justify-between items-center mb-4 border-b border-gray-100 dark:border-gray-700 pb-2 sticky left-0'>
        <h3 className='text-xs font-bold text-gray-500 uppercase tracking-wider'>
          Schematic Straight Line Diagram (SLD)
        </h3>
        <div className='flex gap-4 text-[10px] text-gray-400'>
          <span className='flex items-center gap-1'>
            <span className='w-2 h-2 rounded-full bg-blue-500'></span> Main Route
          </span>
          <span className='flex items-center gap-1'>
            <span className='w-4 border-t border-dashed border-gray-400'></span> Branch Cable
          </span>
        </div>
      </div>
