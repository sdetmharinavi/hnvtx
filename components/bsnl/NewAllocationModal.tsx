...
function PathBuilder({
  ...
  const availableCables = useMemo(
    () => cables.filter((c) => c.sn_id === lastNodeIdInPath || c.en_id === lastNodeIdInPath),
    [cables, lastNodeIdInPath]
  );
  ...
  const handleAddStep = () => {
    if (
      ...
      !nextFiber2 ||
      !lastNodeIdInPath ||
      nextFiber1 === nextFiber2
    )
      return;
    const newSteps = [
      {
        nodeId: lastNodeIdInPath,
        ofcId: nextCableId,
        fiberNumber: parseInt(nextFiber1),
        action: nextAction,
      },
      {
        nodeId: lastNodeIdInPath,
        ofcId: nextCableId,
        fiberNumber: parseInt(nextFiber2),
        action: nextAction,
      },
    ];
    ...
    <div className="space-y-4">
      {path
        ...
        .map((step, pairIndex) => {
          ...
          return (
            <div
              key={pairIndex}
              className="flex items-center bg-gray-50 dark:bg-gray-700/50 p-2 rounded"
            >
              <div className="w-6 h-6 flex items-center justify-center bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-200 rounded-full text-xs font-bold mr-3">
                {pairIndex + 1}
              </div>
              <div className="flex-1 text-sm">
                ...
                <span className="font-bold text-blue-600 dark:text-blue-400 ml-2">
                  F{step.fiberNumber}/{fiber2.fiberNumber}
                </span>
                <span className="ml-3 px-2 py-0.5 text-xs bg-gray-200 dark:bg-gray-600 rounded-full">
      ...
      <div className="border-t border-gray-200 dark:border-gray-600 pt-4">
        <p className="text-sm font-medium text-gray-600 dark:text-gray-300 mb-2">
          Add Step (from {getNodeName(lastNodeIdInPath!)})
        </p>
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
          ...
          <div>
            ...
            <select
              value={nextFiber1}
              onChange={(e) => setNextFiber1(e.target.value)}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm bg-white dark:bg-gray-700 dark:text-white"
              disabled={!nextCableId}
            >
              <option value="" disabled>
                ...
              </option>
              {availableFibers
                .filter((f) => f.toString() !== nextFiber2)
                .map((f) => (
          ...
          <div>
            ...
            <select
              value={nextFiber2}
              onChange={(e) => setNextFiber2(e.target.value)}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm bg-white dark:bg-gray-700 dark:text-white"
              disabled={!nextCableId}
            >
              <option value="" disabled>
              ...
              {availableFibers
                .filter((f) => f.toString() !== nextFiber1)
                .map((f) => (
                  <option key={f} value={f}>
                    {f}
                  </option>
          ...
          <div>
            ...
            <select
              value={nextAction}
              onChange={(e) => setNextAction(e.target.value as 'pass_through' | 'terminate')}
              className="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 text-sm bg-white dark:bg-gray-700 dark:text-white"
            >
        ...
        <button
          ...
          className="mt-4 w-full flex items-center justify-center px-4 py-2 border border-dashed border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"
        >
...
function AdvancedAllocationModal({
  ...
  const resetState = () => {
    setStep(1);
    setSelectedSystemId('');
    setTopology('p2p-unprotected');
    setPaths({ working: [], protection: [], taps: {} });
    setError(null);
  };
  const handleClose = () => {
    resetState();
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Select Allocation Topology
                </label>
                <select
                  value={topology}
                  onChange={(e) => setTopology(e.target.value as AllocationTopology)}
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-md p-2 bg-white dark:bg-gray-700 dark:text-white"
                >
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="border rounded-lg border-gray-200 dark:border-gray-600">
              <div className="p-3 bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-600 font-medium text-gray-700 dark:text-gray-300">
                Working Path
              </div>
              <div className="p-4">
                <PathBuilder
          ...
          <div className="space-x-3">
            ...
            {step < 2 ? (
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:bg-gray-400 dark:disabled:bg-gray-500"
              >
                Next
              </button>
            ) : (
