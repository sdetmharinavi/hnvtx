...
const colorClasses: Record<SwitchColor, { light: string; dark: string }> = {
  primary: { light: 'bg-blue-600', dark: 'bg-blue-500' },
  secondary: { light: 'bg-gray-600', dark: 'bg-gray-500' },
  success: { light: 'bg-green-600', dark: 'bg-green-500' },
  danger: { light: 'bg-red-600', dark: 'bg-red-500' },
  warning: { light: 'bg-yellow-600', dark: 'bg-yellow-500' },
};
    ...
    <div className={`inline-flex items-center gap-3 ${className}`}>
      ...
      <div className='flex items-center gap-2'>
        ...
        <button
          ...
          onClick={handleClick}
          className={`
            ${sizeClasses[size].container} relative inline-flex shrink-0 cursor-pointer rounded-full border-2 border-transparent 
            transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2
            ${checked ? colorClasses[color].light : 'bg-gray-200 dark:bg-gray-600'}
