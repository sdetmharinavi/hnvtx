...
import { FiX, FiBriefcase, FiCalendar, FiEdit3 } from "react-icons/fi";
...
const OfcDetailsModal = ({ ofcCableId, onClose, onEdit }: Props) => {
  const supabase = createClient();

  const { data: ofcCable, isLoading, isError, error } = useTableRecord(
    supabase,
    "ofc_cables",
    ofcCableId,
    {
  ...
  const statusBadge = ofcCable.status ? (
        ...
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gray-50 dark:bg-gray-900/30">
          <div>
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
              {ofcCable.asset_no || "Unnamed OFC Cable"}
            </h2>
            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
              {statusBadge}
              {ofcCable.asset_no && <span>• {ofcCable.asset_no}</span>}
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-gray-900 dark:text-gray-100">{(ofcCable as Row<'ofc_cables'> & { ofc_types: Row<'lookup_types'> })?.ofc_types?.name || "Not set"}</div>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500 dark:text-gray-400 block mb-1">Maintenance Area</label>
                <div className="text-gray-900 dark:text-gray-100">{(ofcCable as Row<'ofc_cables'> & { maintenance_areas: Row<'maintenance_areas'> })?.maintenance_areas?.name || "Not set"}</div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <div className="text-gray-900 dark:text-gray-100">{formatDate(ofcCable.created_at as unknown as string)}</div>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500 dark:text-gray-400 block mb-1">Updated</label>
                <div className="text-gray-900 dark:text-gray-100">{formatDate(ofcCable.updated_at as unknown as string)}</div>
              </div>
            </div>
          </div>
        </div>
...
export default OfcDetailsModal;
