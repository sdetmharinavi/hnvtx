...
export const useRouteGeneration = <T extends FieldValues>({
  ...
  useEffect(() => {
    ...
    if (startingNodeName && endingNodeName) {
      // THE FIX: Explicitly format with " ⇔ " (spaces around the bidirectional arrow)
      // to perfectly match the format: "BJC LASKARPUR ⇔ KAMALGAZI PEYARABAGAN BTS-01_01"
      // Added padStart(2, '0') to make numbers format nicely as _01, _02, etc.
      const formattedRouteNumber = String(routeData.nextRouteNumber).padStart(2, '0');
      const routeName = `${startingNodeName} ⇔ ${endingNodeName}_${formattedRouteNumber}`;
