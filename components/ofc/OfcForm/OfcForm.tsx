...
const OfcForm: React.FC<OfcFormProps> = ({
  ...
  const { data: nodesData, isLoading: nodesLoading } = usePagedData<V_nodes_completeRowSchema>(
    ...
    'v_nodes_complete',
    {
      filters: {
        status: true,
        node_type_name: [...OFC_FORM_CONFIG.ALLOWED_NODE_TYPES],
      },
  ...
  const { data: ofcOwnersData, isLoading: ofcOwnersLoading } = useTableQuery(supabase, 'lookup_types', {
    filters: { category: { operator: 'eq', value: 'OFC_OWNER' }, name: { operator: 'neq', value: 'DEFAULT' } },
    orderBy: [{ column: 'name', ascending: true }], columns: 'id, name', staleTime: OFC_FORM_CONFIG.CACHE_TIME,
  });
  ...
  const startingNodeName = useMemo(() => 
    nodesData?.data?.find(n => n.id === startingNodeId)?.name || null,
    [nodesData, startingNodeId]
  );
  const endingNodeName = useMemo(() => 
    nodesData?.data?.find(n => n.id === endingNodeId)?.name || null,
    [nodesData, endingNodeId]
  );
  ...
  useEffect(() => {
    if (generatedRouteName !== null) {
      setValue('route_name', generatedRouteName, {
        shouldValidate: true,
        shouldDirty: true,
      });
    }
  ...
  const nodeOptions = useMemo((): Option[] =>
    nodesData?.data.map((node: V_nodes_completeRowSchema) => ({ value: String(node.id), label: node.name || `Node ${node.id}` })) || [],
    [nodesData]
  );
  ...
  const isLoading = nodesLoading || ofcTypesLoading || maintenanceTerminalsLoading || pageLoading || routeGenerationLoading || ofcOwnersLoading;
        ...
        <div className="p-6 relative">
          ...
          <div className="space-y-8">
            <RouteConfigurationSection
              control={control} errors={errors} startingNodeOptions={startingNodeOptions}
              endingNodeOptions={endingNodeOptions} routeName={routeName}
            />
            <CableSpecificationsSection
              control={control} register={register} errors={errors}
              setValue={setValue} watch={watch} ofcTypeOptions={ofcTypeOptions}
              ofcOwnerOptions={ofcOwnerOptions} isCapacityLocked={isCapacityLocked}
            />
            <MaintenanceSection
              control={control} errors={errors} maintenanceTerminalOptions={maintenanceTerminalOptions}
            />
          </div>
        </div>
      </FormCard>
