...
import {
  ...
  FormTextarea,
} from "../common/ui/form/FormControls";
...
const OfcForm = ({
  ...
  useEffect(() => {
    const needsFallback = Boolean(
      ofcCable?.id && (!ofcCable.sn_id || !ofcCable.en_id)
    );
    ...
    (async () => {
      const { data, error } = await supabase
        .from("ofc_cables")
        .select(
          "sn_id, en_id, ofc_type_id, asset_no, capacity, commissioned_on, current_rkm, maintenance_terminal_id, status, remark"
        )
        .eq("id", ofcCable!.id)
        .single();
  ...
  useEffect(() => {
    if (
      (startingNodeId && validationErrors.sn_id) ||
      (endingNodeId && validationErrors.en_id)
    ) {
  ...
  const onValidSubmit: SubmitHandler<OfcCableFormData> = (data) => {
  ...
  const { data: nodesData, isLoading: nodesLoading } = usePagedNodesComplete(
    supabase,
    {
      filters: {
        ...
        or: [
          "node_type_name IN ('Terminal Node','Joint / Splice Point','Base Transceiver Station','Backhaul Hub / Block HQ','Customer Premises','Gram Panchayat')",
        ],
      },
      limit: 1000,
    }
  ...
  useEffect(() => {
    ...
    if (startingNodeId && endingNodeId) {
      ...
      setValue(
        "route_name",
        `${startingNodeName}⇔${endingNodeName}_${routeNumber}`
      );
    } else {
  ...
  const endingNodeBase = nodesOptions.filter(
    (option) => option.value !== startingNodeId
  );
  const startingNodeBase = nodesOptions.filter(
    (option) => option.value !== endingNodeId
  );

  const startingNodeLabel =
    nodesData?.find((n) => n.id === startingNodeId)?.name ||
    startingNodeId ||
    undefined;
  const endingNodeLabel =
    nodesData?.find((n) => n.id === endingNodeId)?.name ||
    endingNodeId ||
    undefined;

  const endingNodeOptions: Option[] = ensureOption(
    endingNodeBase,
    endingNodeId,
    endingNodeLabel
  );
  const startingNodeOptions: Option[] = ensureOption(
    startingNodeBase,
    startingNodeId,
    startingNodeLabel
  );
  ...
  const ofcTypeOptions: Option[] = useMemo(
    () =>
    ...
    if (currentOfcTypeId) {
      ...
      if (selectedOption) {
        ...
        if (match) {
          ...
          const matchingCapacityOption = capacityOptions.find(
            (opt) => opt.value === inferredCapacity
          );
          // console.log("matchingCapacityOption", matchingCapacityOption);

          if (matchingCapacityOption) {
            setValue("capacity", Number(matchingCapacityOption.value), {
              shouldValidate: true,
            });
  ...
  }, [capacityOptions, currentOfcTypeId, ofcTypeOptions, setValue]);
  ...
  const { data: existingRoute, isLoading: existingRouteLoading } =
  ...
  useEffect(() => {
    const count = Array.isArray(existingRoute)
      ? typeof existingRoute[0] === "object" &&
        existingRoute[0] !== null &&
        "total_count" in (existingRoute[0] as OfcCableFormData)
        ? ((existingRoute[0] as OfcCableFormData).total_count as number)
        : existingRoute.length
      : 0;
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                    {existingRoutes.length} cable route
                    {existingRoutes.length > 1 ? "s" : ""} already exist between
                    these nodes:
                  </p>
