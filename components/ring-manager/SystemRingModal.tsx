...
export const SystemRingModal: FC<SystemRingModalProps> = ({
  ...
  const systemsOptions = useMemo(() => {
    ...
    return (systemsData || [])
      .filter((s: { id: string | null | undefined; }) => !queuedSystemIds.has(s.id))
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .map((s: { system_name: string; ip_address: any; id: string; }) => {
  ...
  useEffect(() => {
    if (selectedSystemIdForm && systemsData.length > 0) {
      const selectedSystem = systemsData.find((s: { id: string; }) => s.id === selectedSystemIdForm);
