...
export const SystemConnectionFormModal: FC<SystemConnectionFormModalProps> = ({
  ...
  const getPortTypeDisplay = (
    ...
    const port = portsList.find((p: { port: string }) => p.port === portInterface);
  ...
  useEffect(() => {
    if (watchSnId && systemsData) {
      const sys = systemsData.find((s: { id: string }) => s.id === watchSnId);
  ...
  useEffect(() => {
    if (watchEnId && systemsData) {
      const sys = systemsData.find((s: { id: string }) => s.id === watchEnId);
