...
const PathBuilder: FC<{
  ...
  const lastNode = useMemo(() => {
    ...
    // Only traverse valid steps to find the current end of the path
  ...
  return (
    <div className='space-y-3'>
      {fields.map((field, index) => {
        ...
        // Re-calculate previous node for this specific step to display correct segment info
      ...
      <div className='space-y-2 pt-3 mt-3 border-t dark:border-gray-600'>
        <p className='text-xs font-medium text-gray-500 dark:text-gray-400'>
          ...
          <span className='font-bold text-gray-700 dark:text-gray-300'>
            {lastNode?.name || startNode?.name || 'Unknown Node'}
          </span>
        </p>
        <div className='flex items-center gap-2'>
          <div className='grow'>
            <SearchableSelect
              ...
              placeholder={
                !lastNode
                  ? 'No starting node identified'
                  : availableCables.length === 0
                    ? 'No cables available'
                    : 'Search for next cable...'
              }
              disabled={!lastNode}
              clearable
            />
