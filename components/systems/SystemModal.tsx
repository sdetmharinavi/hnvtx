...
export const SystemModal: FC<SystemModalProps> = ({
  ...
  useEffect(() => {
    ...
    const autoFillTerminal = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from('v_nodes_complete')
          .select('maintenance_terminal_id, maintenance_area_name')
          .eq('id', selectedNodeId)
          .single();

        if (error) throw error;

        if (data && data.maintenance_terminal_id) {
          if (data.maintenance_area_name) {
            setInferredTerminalOption({
              value: data.maintenance_terminal_id,
              label: data.maintenance_area_name,
            });
          }
          setValue('maintenance_terminal_id', data.maintenance_terminal_id, {
            shouldValidate: true,
            shouldDirty: true,
          });
        }
    ...
    autoFillTerminal();
  }, [selectedNodeId, setValue, isEditMode, rowData, supabase]);
  ...
  return (
    <BaseFormModal
      ...
      form={form}
      onSubmit={(data) => onValidSubmit(data)}
      widthClass='w-full max-w-[95vw]'
      heightClass='h-full'
