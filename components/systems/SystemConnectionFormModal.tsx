...
export const SystemConnectionFormModal: FC<SystemConnectionFormModalProps> = ({ isOpen, onClose, parentSystem, editingConnection, onSubmit, isLoading }) => {
  ...
  const { data: portTypes = { data: [] } } = useTableQuery(supabase, "lookup_types", { columns: "id, name", filters: { category: "PORT_TYPES" } });
  ...
  const portTypeOptions = useMemo(() => portTypes.data.map((t) => ({ value: t.id, label: t.name })), [portTypes]);
  ...
  const renderFooter = () => {
    ...
    return (
      <div className="flex justify-between w-full">
        {step === 2 && <Button type="button" variant="outline" onClick={() => setStep(1)} disabled={isLoading}>Back</Button>}
