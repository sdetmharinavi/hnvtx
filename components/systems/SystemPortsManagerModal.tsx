...
function usePortServicesLogic(systemId: string | null, isOpen: boolean) {
  const supabase = createClient();
  const { data: connectionsResult } = usePagedData<V_system_connections_completeRowSchema>(
    supabase,
    'v_system_connections_complete',
    {
      filters: { or: `system_id = '${systemId}' OR en_id = '${systemId}'` },
      limit: 2000,
    },
    { enabled: !!systemId && isOpen },
  );

  return useMemo((): PortServiceMap => {
...
function usePortStatsLogic(ports: V_ports_management_completeRowSchema[]) {
  return useMemo(() => {
...
export const SystemPortsManagerModal: React.FC<SystemPortsManagerModalProps> = ({
  ...
  // 1. Fetch Ports via CRUD Manager
  const {
    data: ports,
    totalCount,
    isLoading,
    ...
    deleteModal,
    actions: crudActions,
    bulkActions, // Added bulkActions for proper prop mapping
    ...
    statusColumn: 'port_admin_status', // THE FIX: Map the specific column name
  });
  ...
  const portServicesMap = usePortServicesLogic(systemId, isOpen);
  const portStats = usePortStatsLogic(ports);

  // 3. Prepare Filter Config
  ...
  const handleApplyTemplate = useCallback(
    (templateKey: string) => {
      ...
      bulkUpsert.mutate(
        ...
        {
          onSuccess: () => {
            toast.success(`Successfully populated ${portsPayload.length} ports.`);
            refetch();
            setIsTemplateModalOpen(false);
          },
          onError: (err) => toast.error(`Failed to populate: ${err.message}`),
        },
      );
    },
  ...
  const tableActions = useMemo(
    (): TableAction<'v_ports_management_complete'>[] =>
      createStandardActions<V_ports_management_completeRowSchema>({
        ...
        // THE FIX: Directly pass the record; the hook now knows the column name
        onToggleStatus: crudActions.handleToggleStatus,
      }),
    [editModal.openEdit, crudActions, canDelete],
  );
    ...
    >
      <div className='space-y-4'>
        ...
        <GenericFilterBar
          ...
          // ADDED: Extra actions section for bulk actions UI inside modal
          extraActions={
            bulkActions.selectedCount > 0 && (
              <div className='flex gap-2'>
                <button
                  className='px-2 py-1 bg-green-50 text-green-700 text-xs rounded border border-green-200'
                  onClick={() => bulkActions.handleBulkUpdateStatus('active')}
                >
                  Mark UP
                </button>
                <button
                  className='px-2 py-1 bg-red-50 text-red-700 text-xs rounded border border-red-200'
                  onClick={() => bulkActions.handleBulkUpdateStatus('inactive')}
                >
                  Mark DOWN
                </button>
                {canDelete && (
                  <button
                    className='px-2 py-1 bg-gray-50 text-red-700 text-xs rounded border border-red-200'
        ...
        <DataTable
          ...
          selectable={true}
          onRowSelect={(rows) => {
            // const validIds = rows.map((r) => r.id).filter((id) => id !== null) as string[];
            // Cheat the type since the useCrudManager creates a wrapper function
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            (bulkActions as any).handleRowSelect(rows);
          }}
          pagination={{
