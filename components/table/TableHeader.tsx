...
interface TableHeaderProps<T extends AuthTableOrViewName> extends Pick<DataTableProps<T>, 
  | 'columns'
  | 'selectable'
  | 'sortable'
  | 'bordered'
      ...
      <tr>
        ...
        {visibleColumns.map((column, index) => (
          <th
            ...
            } ${bordered ? `border-b ${index < visibleColumns.length - 1 || hasActions ? "border-r" : ""} border-gray-200 dark:border-gray-700` : ""}`}
            style={{ 
              width: column.width,
            ...
            <div className='flex items-center gap-1 sm:gap-2 min-w-0'>
              ...
              {column.sortable && sortable && (
                <div className='flex flex-col flex-shrink-0'>
                  <FiArrowUp 
                    size={10} 
                    className={`sm:size-3 ${sortConfig?.key === column.dataIndex && sortConfig.direction === "asc" ? "text-blue-600 dark:text-blue-400" : "text-gray-300 dark:text-gray-600"}`} 
                  />
                  <FiArrowDown 
                    size={10} 
                    className={`-mt-0.5 sm:size-3 sm:-mt-1 ${sortConfig?.key === column.dataIndex && sortConfig.direction === "desc" ? "text-blue-600 dark:text-blue-400" : "text-gray-300 dark:text-gray-600"}`} 
                  />
                </div>
              )}
            </div>
        ...
        {hasActions && (
          <th className={`w-20 sm:w-auto ${densityClasses[density ?? "default"]} text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider ${bordered ? "border-b border-gray-200 dark:border-gray-700" : ""}`}>
