...
SELECT
  ...
  ma.name AS maintenance_area_name,
  -- Aggregated system names for search
  (
  ...
  -- NEW COLUMN: Aggregated system IPs for search
  (
    SELECT string_agg(host(s.ip_address), ', ')
    FROM public.ring_based_systems rbs
    JOIN public.systems s ON rbs.system_id = s.id
    WHERE rbs.ring_id = r.id AND s.ip_address IS NOT NULL
  ) as associated_system_ips
FROM public.rings r
