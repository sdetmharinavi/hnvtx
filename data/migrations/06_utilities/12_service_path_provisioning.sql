...
-- 2. Update provision_service_path to use 'Provisioned'
...
DECLARE
    v_system_id UUID;
    v_en_id UUID;
    v_provisioned_status_id UUID; -- Changed variable name
    v_link_type_id UUID;
...
BEGIN
    ...
    -- [CHANGE]: Look for 'Provisioned' instead of 'active'
    ...
    -- Fallback to 'Active' if Provisioned doesn't exist
    ...
    -- 4. Create Working Path
    INSERT INTO public.logical_fiber_paths (
        path_name, source_system_id, destination_system_id, source_port, destination_port,
        path_type_id, path_role, operational_status_id, system_connection_id
    ) VALUES (
        p_path_name || ' (Working)', v_system_id, v_en_id, v_sn_interface, v_en_interface,
        v_link_type_id, 'working', v_provisioned_status_id, p_system_connection_id
    ) RETURNING id INTO v_working_path_id;

    -- 5. Assign Working Fibers
    UPDATE public.ofc_connections
    ...
    SELECT v_working_path_id, oc.ofc_id, ROW_NUMBER() OVER (ORDER BY MIN(t.ord))::int
    FROM unnest(p_working_tx_fiber_ids) WITH ORDINALITY as t(fiber_id, ord)
    JOIN public.ofc_connections oc ON oc.id = t.fiber_id
    GROUP BY oc.ofc_id;

    -- 6. Create Protection Path
    IF array_length(p_protection_tx_fiber_ids, 1) > 0 THEN
        INSERT INTO public.logical_fiber_paths (
            path_name, source_system_id, destination_system_id, source_port, destination_port,
            path_type_id, path_role, working_path_id, operational_status_id, system_connection_id
        ) VALUES (
            p_path_name || ' (Protection)', v_system_id, v_en_id, v_sn_protection_interface, v_en_protection_interface,
            v_link_type_id, 'protection', v_working_path_id, v_provisioned_status_id, p_system_connection_id
        ...
        INSERT INTO public.logical_path_segments (logical_path_id, ofc_cable_id, path_order)
        SELECT v_protection_path_id, oc.ofc_id, ROW_NUMBER() OVER (ORDER BY MIN(t.ord))::int
