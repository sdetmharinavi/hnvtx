<!-- path: telecom_network_db/04_advanced_ofc/1_tables/1_fiber_joints.sql -->
```sql
...
<!-- path: telecom_network_db/04_advanced_ofc/1_tables/3_fiber_joint_connections.sql -->
```sql
...
<!-- path: telecom_network_db/04_advanced_ofc/1_tables/2_logical_fiber_paths.sql -->
```sql
...
<!-- path: telecom_network_db/03_network_systems/1_tables/09_sdh_connections.sql -->
```sql
...
<!-- path: telecom_network_db/03_network_systems/1_tables/08_sdh_systems.sql -->
```sql
...
<!-- path: telecom_network_db/03_network_systems/1_tables/11_vmux_systems.sql -->
```sql
...
<!-- path: telecom_network_db/03_network_systems/1_tables/12_vmux_connections.sql -->
```sql
...
<!-- path: telecom_network_db/03_network_systems/1_tables/10_sdh_node_associations.sql -->
```sql
...
<!-- path: telecom_network_db/03_network_systems/1_tables/03_consolidated_tables.sql -->
```sql
...
```sql
-- REFACTORED: This view now joins the new 'sfp_based_connections' table.
...
select
  ...
  na.name as sn_node_name, -- Added for completeness
  sc.sn_ip,
  sc.sn_interface,
  s_en.system_name as en_name,
  nb.name as en_node_name, -- Added for completeness
  sc.en_ip,
  sc.en_interface,
  ...
  sc.created_at,
  sc.updated_at,
  -- UPDATED: All columns now come from the single 'sfp_based_connections' table
  sfpc.sfp_port as maan_sfp_port,
  lt_sfp.name as maan_sfp_type_name,
  sfpc.sfp_capacity as maan_sfp_capacity,
  sfpc.sfp_serial_no as maan_sfp_serial_no,
  sfpc.fiber_in as maan_fiber_in,
  sfpc.fiber_out as maan_fiber_out,
  sfpc.customer_name as maan_customer_name,
  sfpc.bandwidth_allocated_mbps as maan_bandwidth_allocated_mbps,
  -- SDH connection details
  scs.stm_no as sdh_stm_no,
  -- ... (rest of SDH/VMUX columns are unchanged)
  vcs.tk as vmux_tk,
  count(*) OVER() AS total_count,
...
from system_connections sc
  ...
  -- UPDATED: Replaced joins to maan_connections/cpan_connections with a single join
