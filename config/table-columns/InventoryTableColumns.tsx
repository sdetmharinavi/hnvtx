...
export const getInventoryTableColumns = (): Column<V_inventory_itemsRowSchema>[] => [
  ...
  },
  {
    ...
    render: (val) => (
      <span
        className={`font-bold ${
          Number(val) === 0 ? 'text-red-500' : 'text-gray-900 dark:text-white'
        }`}
      >
        {val as number}
      </span>
  ...
  },
  {
    ...
    render: (val) =>
      val ? (
        <span className="text-gray-600 dark:text-gray-400">{formatCurrency(Number(val))}</span>
      ) : (
        '-'
      ),
  },
      ...
      val ? (
        <div className="flex flex-col text-xs">
          ...
          <span className="text-gray-500">
            {formatDate(record?.last_issued_date as string, { format: 'dd-mm-yyyy' })}
          </span>
        </div>
      ) : (
