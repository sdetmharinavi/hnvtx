...
export const ofcDetailsConfig = {
  header: {
    ...
    avatar: {
      ...
      fallbackText: (ofc: V_ofc_cables_completeRowSchema) =>
        ofc.ofc_owner_name?.charAt(0)?.toUpperCase() || 'O',
    },
    badges: [
      {
        key: 'ofc_type_name',
      ...
      {
        key: 'status',
        component: (status: boolean) => <StatusBadge status={status ? 'ACTIVE' : 'INACTIVE'} />,
      },
    ],
...
export const OfcDetailsModal = ({
  ofc,
  onClose,
  isOpen,
}: {
  ofc: V_ofc_cables_completeRowSchema;
  onClose: () => void;
  isOpen: boolean;
}) => {
  return <DetailsModal data={ofc} onClose={onClose} isOpen={isOpen} config={ofcDetailsConfig} />;
};
