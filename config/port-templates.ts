  ...
  '3beb3ea2-55a4-48da-a7fa-f7c9ccf7de79': {
    ...
    ports: [
      ...
      ...Array.from({ length: 16 }, (_, i) =>
        createPort(`1.${i + 5}`, PORT_TYPES.E1_2MBPS, '2mbps')
      ),

      // Slot 2 (STM1 & E1)
      ...['2.1', '2.2', '2.3', '2.4'].map((p) => createPort(p, PORT_TYPES.STM1, 'STM1')),
      ...Array.from({ length: 16 }, (_, i) =>
        createPort(`2.${i + 5}`, PORT_TYPES.E1_2MBPS, '2mbps')
      ),

      // Slot 3 (10GE & GE)
      createPort('3.1', PORT_TYPES.TEN_GE, '10GE'),
      ...Array.from({ length: 5 }, (_, i) =>
        createPort(`3.${i + 2}`, PORT_TYPES.GE_OPTICAL, 'GE(O)')
      ),
      ...Array.from({ length: 6 }, (_, i) =>
        createPort(`3.${i + 7}`, PORT_TYPES.GE_ELECTRICAL, 'GE(E)')
      ),

      // Slot 4 (10GE & GE)
      createPort('4.1', PORT_TYPES.TEN_GE, '10GE'),
      ...Array.from({ length: 5 }, (_, i) =>
        createPort(`4.${i + 2}`, PORT_TYPES.GE_OPTICAL, 'GE(O)')
      ),
      ...Array.from({ length: 6 }, (_, i) =>
        createPort(`4.${i + 7}`, PORT_TYPES.GE_ELECTRICAL, 'GE(E)')
      ),

      // Slot 5 (10GE & GE)
      createPort('5.1', PORT_TYPES.TEN_GE, '10GE'),
      ...Array.from({ length: 5 }, (_, i) =>
        createPort(`5.${i + 2}`, PORT_TYPES.GE_OPTICAL, 'GE(O)')
      ),
      ...Array.from({ length: 6 }, (_, i) =>
        createPort(`5.${i + 7}`, PORT_TYPES.GE_ELECTRICAL, 'GE(E)')
      ),

      // Slot 6 (10GE & GE)
      createPort('6.1', PORT_TYPES.TEN_GE, '10GE'),
      ...Array.from({ length: 5 }, (_, i) =>
        createPort(`6.${i + 2}`, PORT_TYPES.GE_OPTICAL, 'GE(O)')
      ),
      ...Array.from({ length: 6 }, (_, i) =>
        createPort(`6.${i + 7}`, PORT_TYPES.GE_ELECTRICAL, 'GE(E)')
      ),
  ...
  'ce06f9b7-02e7-4741-8911-46cf1f47ffdd': {
    ...
    ports: [
      ...Array.from({ length: 12 }, (_, i) =>
        createPort(`ETH-1-1-${i + 1}`, PORT_TYPES.GE_OPTICAL, 'GE(O)')
      ),
      ...['13', '14', '15', '16'].map((n) => createPort(`ETH-1-1-${n}`, PORT_TYPES.TEN_GE, '10GE')),
      ...['17', '18', '19', '20'].map((n) =>
        createPort(`ETH-1-1-${n}`, PORT_TYPES.GE_OPTICAL, 'GE(E)')
      ),
      createPort('NMS', PORT_TYPES.FE, 'FE'),
    ],
  },
  ...
  '2efeaec3-25db-4e92-bb1b-0ce370547cd6': {
    ...
    ports: [
      ...
      ...Array.from({ length: 10 }, (_, i) =>
        createPort(`ETH-1-1-${i + 4}`, PORT_TYPES.TEN_GE, '10G')
      ),

      // 1G Ports (14 to 31)
      ...Array.from({ length: 18 }, (_, i) =>
        createPort(`ETH-1-1-${i + 14}`, PORT_TYPES.GE_OPTICAL, '1G')
      ),
  ...
  '8be22dce-38be-47d4-a1cb-59749b7c9b07': {
    ...
    ports: [
      ...
      ...Array.from({ length: 4 }, (_, i) =>
        createPort(`ETH-1-4-${i + 1}`, PORT_TYPES.TEN_GE, '10G')
      ),
      ...Array.from({ length: 2 }, (_, i) =>
        createPort(`ETH-1-4-${i + 5}`, PORT_TYPES.HUNDRED_GE, '100G')
      ),

      // Slot 5: ETH-1-5-1 to 10 (10G), 11 (100G)
      ...Array.from({ length: 10 }, (_, i) =>
        createPort(`ETH-1-5-${i + 1}`, PORT_TYPES.TEN_GE, '10G')
      ),
      createPort('ETH-1-5-11', PORT_TYPES.HUNDRED_GE, '100G'),

      // Slot 6: ETH-1-6-1 to 10 (10G), 11 (100G)
      ...Array.from({ length: 10 }, (_, i) =>
        createPort(`ETH-1-6-${i + 1}`, PORT_TYPES.TEN_GE, '10G')
      ),
      createPort('ETH-1-6-11', PORT_TYPES.HUNDRED_GE, '100G'),

      // Slot 7: ETH-1-7-1 to 8 (10G)
      ...Array.from({ length: 8 }, (_, i) =>
        createPort(`ETH-1-7-${i + 1}`, PORT_TYPES.TEN_GE, '10G')
      ),

      // Slot 12: ETH-1-12-1 to 4 (1000 BaseT), STM1-1-12-5 to 8
      ...Array.from({ length: 4 }, (_, i) =>
        createPort(`ETH-1-12-${i + 1}`, PORT_TYPES.GE_ELECTRICAL, '1000 BaseT')
      ),
      ...Array.from({ length: 4 }, (_, i) =>
        createPort(`STM1-1-12-${i + 5}`, PORT_TYPES.STM1, 'STM1')
      ),

      // Slot 15: ETH-1-15-1 to 8 (1G)
      ...Array.from({ length: 8 }, (_, i) =>
        createPort(`ETH-1-15-${i + 1}`, PORT_TYPES.GE_OPTICAL, '1G')
      ),

      // Slot 16: ETH-1-16-1 to 8 (1G)
      ...Array.from({ length: 8 }, (_, i) =>
        createPort(`ETH-1-16-${i + 1}`, PORT_TYPES.GE_OPTICAL, '1G')
      ),

      // Slot 17: ETH-1-17-1 to 8 (1G)
      ...Array.from({ length: 8 }, (_, i) =>
        createPort(`ETH-1-17-${i + 1}`, PORT_TYPES.GE_OPTICAL, '1G')
      ),

      // Slot 18: ETH-1-18-1 to 8 (1G)
      ...Array.from({ length: 8 }, (_, i) =>
        createPort(`ETH-1-18-${i + 1}`, PORT_TYPES.GE_OPTICAL, '1G')
      ),
    ],
  },
};

export const generatePortsFromTemplate = (
  templateKey: string,
  systemId: string
