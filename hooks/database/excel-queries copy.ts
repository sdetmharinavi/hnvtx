...
export function useExcelUpload<T extends TableName>(
  ...
  return useMutation<UploadResult, Error, UploadOptions<T>>({
    mutationFn: async (uploadOptions: UploadOptions<T>): Promise<UploadResult> => {
      ...
      // Filter out rows that are empty across all non-id columns
      const filteredRows = dataRows.filter((row) => !isRowEffectivelyEmpty(row));

      const recordsToProcess = filteredRows.map((row): TableInsert<T> => {
        ...
        columns.forEach((mapping) => {
          ...
          if (mapping.dbKey === 'id' && isFirstColumnId && (rawValue === null || rawValue === undefined || String(rawValue).trim() === '')) {
            rawValue = generateUUID();
          }

          // If 'id' header is entirely missing, still generate a UUID
          if (mapping.dbKey === 'id' && colIndex === undefined) {
            rawValue = generateUUID();
          }
          ...
          // Preserve non-id empty strings; only set null if both finalValue and rawValue are undefined
          (record as any)[mapping.dbKey] = finalValue !== undefined ? finalValue : (rawValue !== undefined ? rawValue : null);
        });
