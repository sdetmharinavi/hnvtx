...
export function useToggleStatus<T extends PublicTableName>(
  ...
  options?: UseTableMutationOptions<
    TableRow<T>,
    { id: string; status: boolean; nameField?: keyof TableRow<T>; statusField?: string },
    OptimisticContext
  >,
) {
  ...
  return useMutation<
    ...
    OptimisticContext
  >({
    mutationFn: async ({ id, status, statusField = 'status' }): Promise<TableRow<T>> => {
      const updateData = { [statusField]: status, updated_at: new Date().toISOString() };
    ...
    onMutate: optimisticUpdate
      ? async ({ id, status, statusField = 'status' }) => {
          ...
          queryClient.setQueriesData({ queryKey: ['table', tableName] }, (old: any) => {
            ...
            const updateItem = (item: any) =>
              'id' in item && item.id === id
                ? { ...item, [statusField]: status, updated_at: new Date().toISOString() }
                : item;
...
export function useTableUpdate<T extends PublicTableName>(
  ...
  return useMutation<TableRow<T>[], Error, { id: string; data: TableUpdate<T> }, OptimisticContext>(
    {
      mutationFn: async ({ id, data }): Promise<TableRow<T>[]> => {
      ...
      onMutate: optimisticUpdate
        ? async ({ id, data: newData }) => {
            ...
            queryClient.setQueriesData({ queryKey: ['table', tableName] }, (old: any) => {
              if (!old) return old;
              if (isPagedResult(old)) return { ...old, data: old.data.map(updateItem) };
