  ...
  return useMutation<EnhancedUploadResult, Error, RingUploadOptions>({
    mutationFn: async ({ file }): Promise<EnhancedUploadResult> => {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        const topologyConfigRaw = row[headerMap.get('topology_config') ?? -1]; 
    ...
    onSuccess: (result) => {
      ...
      // THE FIX: Implemented master cache invalidator
      invalidateRelatedCaches(queryClient, 'rings');
      invalidateRelatedCaches(queryClient, 'systems');
    },
    onError: (error) => {
