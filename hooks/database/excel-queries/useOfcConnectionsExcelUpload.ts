    ...
    mutationFn: async (uploadOptions): Promise<EnhancedUploadResult> => {
      ...
      for (let i = 0; i < dataRows.length; i++) {
        ...
        if (!fiberSn || fiberSn <= 0) {
             rowValidationErrors.push({ rowIndex: i, column: 'fiber_no_sn', value: processedData.fiber_no_sn, error: 'Invalid Fiber No' });
        }

        const fiberEn = processedData.fiber_no_en ? parseExcelInt(processedData.fiber_no_en) : fiberSn;
        const updatedFiberSn = processedData.updated_fiber_no_sn ? parseExcelInt(processedData.updated_fiber_no_sn) : fiberSn;
        const updatedFiberEn = processedData.updated_fiber_no_en ? parseExcelInt(processedData.updated_fiber_no_en) : fiberEn;
        
        // Only parse segment order if present
        const pathSegmentOrder = processedData.path_segment_order ? parseExcelInt(processedData.path_segment_order) : null;
      ...
      if (recordsToUpdate.length > 0) {
          ...
          if (updates.length > 0) {
             ...
             const { error: updateError } = await supabase.from('ofc_connections').upsert(updates as any, { 
          ...
          if (inserts.length > 0) {
             // eslint-disable-next-line @typescript-eslint/no-explicit-any
             const { error: insertError } = await supabase.from('ofc_connections').insert(inserts as any);
             if (insertError) {
      ...
      if (uploadResult.successCount > 0) {
          if (showToasts) toast.success(`Successfully processed ${uploadResult.successCount} fibers.`);
      }
