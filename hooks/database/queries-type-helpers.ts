...
export type TableInsertWithDates<T extends TableName> = {
  [K in keyof TableInsert<T>]?: TableInsert<T>[K] | Date | null;
};
export type TableUpdateWithDates<T extends TableName> = {
  [K in keyof TableUpdate<T>]?: TableUpdate<T>[K] | Date | null;
};
...
export interface EnhancedOrderBy {
  column: string;
  ascending?: boolean;
  nullsFirst?: boolean;
  foreignTable?: string;
...
export interface UseUniqueValuesOptions<T extends TableOrViewName, TData = unknown[]>
  extends Omit<UseQueryOptions<unknown[], Error, TData>, 'queryKey' | 'queryFn'> {
  tableName: T;
  filters?: Filters;
  orderBy?: OrderBy[];
  limit?: number;
...
export interface UseRpcQueryOptions<T extends RpcFunctionName, TData = RpcFunctionReturns<T>>
  extends Omit<UseQueryOptions<RpcFunctionReturns<T>, Error, TData>, 'queryKey' | 'queryFn'> {
  performance?: PerformanceOptions;
}
export interface UseTableMutationOptions<TData = unknown, TVariables = unknown, TContext = unknown>
