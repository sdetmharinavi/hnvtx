...
type CableForConnectionCreation = Pick<Ofc_cablesRowSchema, 'id' | 'capacity' | 'sn_id' | 'en_id'>;
...
export const useCreateOfcConnection = () => {
  ...
  return useMutation({
    mutationFn: async ({ cable }: { cable: CableForConnectionCreation }) => {
    ...
    onSuccess: (result) => {
      if (result.count > 0) {
        ...
        // THE FIX: Use master invalidator to automatically hit the views as well.
