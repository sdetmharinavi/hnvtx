...
export function useEFiles(filters?: { status?: string }) {
  return useQuery({
    ...
    queryFn: async (): Promise<V_e_files_extendedRowSchema[]> => {
...
export function useInitiateFile() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      invalidateRelatedCaches(queryClient, 'e_files');
      invalidateRelatedCaches(queryClient, 'file_movements');
    },
